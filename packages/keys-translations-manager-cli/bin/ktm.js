...
var path = require('path'),
  ...
  document2FileContent = require('keys-translations-manager-core/lib/transformationUtil')
...
if (!['export', 'reset'].includes(command)) {
  log(
    'error',
    `Unexpected command "${command}". Use "ktm -h" for more detail.`
  );
  process.exit(1);
}
    ...
    async function () {
      if (command === 'reset') {
        ...
        const getCollectionName = () => {
          const ktmRelease = process.env.KTM_RELEASE;
          if (ktmRelease) {
            if (!releases.includes(ktmRelease)) {
              log('error', `${ktmRelease} not found, use 'latest' instead`);
              return 'latest';
            }
            return ktmRelease;
          }
        ...
        const release = getCollectionName();
        log('info', `process.env.KTM_RELEASE ${process.env.KTM_RELEASE}\n`);
        log('info', `export ${release}\n`);
        ...
        while (lenOutputs--) {
          ...
          locales.forEach(function (locale) {
            var formatted = output.formatted,
              ...
              file = path.join(
                filePath,
                output.filename.replace('${locale}', locale) +
                  `.${finalFileType}`
              ),
              query,
              criteria = {
